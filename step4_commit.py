...
def commit_regenerated_files(pr_info, regenerated_files):
    ...
    try:
        ...
        for fname, data in regenerated_files.items():
            ...
            if 'LLM-corrected build errors' in changes or 'build dependency errors' in changes:
        ...
        if llm_build_corrections > 0:
            print(f"[Step4] 🔧 LLM Build Corrections: {llm_build_corrections} files auto-corrected after build failures (including dependency feedback loops)")
...
**🤖 LLM npm Corrections:** {llm_npm_corrections} package.json files processed with intelligent error correction (loop-based fixing until npm install succeeds)"""
                    ...
                    if llm_build_corrections > 0:
                        pr_body += f"""
**🔧 LLM Build Corrections:** {llm_build_corrections} files processed with intelligent build error correction including dependency feedback loops (package.json updates when build reveals missing dependencies)"""
...
- 🔄 Dependency feedback loops (build errors trigger package.json updates → npm install → retry build)
